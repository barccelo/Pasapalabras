...
export default function Home() {
      ...
      <section className="setup-wrap">
        ...
        <div className="setup-card timer-card">
          ...
          {timerMode === "countdown" && <div className="time-inputs">
            <SwipeNumberInput label="minutos" value={Math.floor(duration / 60)} min={0} max={59} onChange={(value) => updateDuration(value, duration % 60)} />
            <b>:</b>
            <SwipeNumberInput label="segundos" value={duration % 60} min={0} max={59} onChange={(value) => updateDuration(Math.floor(duration / 60), value)} />
          </div>}
        </div>
  ...
  if (view === "done") {
    ...
    return <main className="message-page summary-page"><section className="message-card results-card"><span className="turn-done">★</span><small>Partida terminada</small><h1>{mode === "teams" ? `${ranked[0]?.name} gana` : "Resultado final"}</h1><p>{mode === "teams" ? "Gana quien consigue más aciertos; en caso de empate, quien usa menos tiempo." : "Así terminó tu rosco."}</p><div className="team-results flip-results">{ranked.map((result, index) => { const flipped = flippedResults.includes(index); return <div className={`flip-result${flipped ? " flipped" : ""}`} key={result.name} onClick={() => setFlippedResults((items) => items.includes(index) ? items.filter((item) => item !== index) : [...items, index])}><div className="flip-result-inner"><section className={`flip-result-front${index === 0 ? " winner" : ""}`}><span>{index === 0 && mode === "teams" ? "Ganador" : "Resultado"}</span><h2>{result.name}</h2><strong>{result.correct} <small>aciertos</small></strong><p>{clock(result.elapsed)} · {result.wrong} incorrectas</p><em>Toca para ver el rosco ↻</em></section><section className="flip-result-back"><div className="mini-result-rosco">{result.words.map((word, wordIndex) => { const angle = (360 / result.words.length) * wordIndex - 90; return <button key={wordIndex} className={word.mark} style={{ "--angle": `${angle}deg` } as CSSProperties} onClick={(event) => { event.stopPropagation(); setSummarySelection({ result: index, word: wordIndex }); }}>{word.letter}</button>; })}<div><strong>{result.correct}</strong><small>aciertos</small></div></div></section></div></div>; })}</div>
      ...
      {selected && <div className="answer-detail" role="dialog" aria-modal="true" onClick={() => setSummarySelection(null)}><section className="swipeable-detail" onClick={(event) => event.stopPropagation()} onTouchStart={(event) => { summarySwipeStartRef.current = event.touches[0]?.clientX ?? null; }} onTouchEnd={(event) => { if (summarySwipeStartRef.current === null) return; const distance = (event.changedTouches[0]?.clientX ?? summarySwipeStartRef.current) - summarySwipeStartRef.current; summarySwipeStartRef.current = null; if (distance > 35) navigateCategory(-1); else if (distance < -35) navigateCategory(1); }} onTouchCancel={() => { summarySwipeStartRef.current = null; }}><button onClick={() => setSummarySelection(null)} aria-label="Cerrar">×</button><span className={`detail-letter ${selected.mark}`}>{selected.letter}</span><small>{selected.relation === "EMPIEZA" ? "Empieza por" : "Contiene la"} {selected.letter}</small><h2>{selected.clue}</h2><div className="detail-answer"><span>Respuesta</span><strong>{selected.answer}</strong></div><div className="detail-navigation"><button onClick={() => navigateCategory(-1)} disabled={categoryPosition <= 0} aria-label="Respuesta anterior">‹</button><span>{categoryPosition + 1} de {categoryWordIndexes.length}</span><button onClick={() => navigateCategory(1)} disabled={categoryPosition >= categoryWordIndexes.length - 1} aria-label="Respuesta siguiente">›</button></div></section></div>}
...
function SwipeNumberInput({ label, value, min, max, onChange }: { label: string; value: number; min: number; max: number; onChange: (value: number) => void }) {
  ...
  const handlePointerMove = (event: ReactPointerEvent<HTMLInputElement>) => {
    ...
    const steps = Math.round((drag.current.y - event.clientY) / 24);
    onChange(clamp(drag.current.value + steps));
  };
  const stopDragging = () => { drag.current = null; };
  return <label className="swipe-number"><input type="number" inputMode="numeric" min={min} max={max} value={value} onChange={(event) => onChange(clamp(Number(event.target.value)))} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={stopDragging} onPointerCancel={stopDragging} onWheel={(event) => { event.preventDefault(); onChange(clamp(value + (event.deltaY < 0 ? 1 : -1))); }} /><span>{label}</span></label>;
}
